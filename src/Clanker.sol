...
contract Clanker is OwnerAdmins, ReentrancyGuard, IClanker {
    ...
    // cc0strategy: FeeDistributor for NFT holder rewards
    IFeeDistributor public immutable feeDistributor;

    // cc0strategy: error for invalid NFT collection
    error InvalidNftCollection();

    constructor(address owner_, address feeDistributor_) OwnerAdmins(owner_) {
        ...
        returns (address tokenAddress)
    {
        ...
        // cc0strategy: Register token with FeeDistributor for NFT holder rewards
        // cc0strategy: NFT collection is REQUIRED
        if (deploymentConfig.nftCollection == address(0)) revert InvalidNftCollection();
        {
            uint256 nftSupply = IERC721Enumerable(deploymentConfig.nftCollection).totalSupply();
            if (nftSupply == 0) revert InvalidNftCollection();
            feeDistributor.registerToken(
                tokenAddress,
                deploymentConfig.nftCollection,
